...
- **terraform/modules/incident-io-alert-source**: New module — incident.io alert source as code via the official `incident-io/incident` provider (`>= 5.39, < 6.0` — the floor is the release that compares engine param-binding `literal` JSON semantically instead of byte-wise, which the module's `jsonencode()`'d default templates rely on for plan stability). Owns the non-obvious piece: default title/description payload templates as the rich-text engine-expression (TipTap/varSpec) documents the incident.io editor generates, binding Payload → Title / Payload → Description — correct out of the box for Grafana webhook contact-point payloads (`source_type = "grafana"`, the default) and other webhook-style sources (`http`, `cloudwatch`, …). Exports `alert_events_url` + `secret_token` (both sensitive) so alerting producers — e.g. `grafana-cloud-alerting`'s `webhook_url`/`webhook_token` — consume the webhook credential through state instead of a human copy-pasting it from the UI. Template overrides (`title_template`, `description_template`, `attributes`, `expressions`) accept the incident.io UI's Terraform-export output verbatim for richer payload mappings. Follows the repo's `enabled` count-gating convention (cold-start/credential-less plans create nothing and never touch the incident.io API). Alert ROUTES are deliberately not wrapped: their schema is deep and organization-specific — author in the incident.io visual editor, export, and commit alongside (the module's `id` output is the `alert_source_id` routes bind to). Provider auth + least-privilege API-key bundles ("View data" + "Create and manage on-call resources") documented in `provider.tf`; secret_token rotation semantics (taint the resource; rotating the API key does NOT rotate source tokens) documented in `resources.tf`.
- **terraform/examples/incident-io-alert-source**: Composed example pairing the new module with `grafana-cloud-alerting`: `require_webhook = false` deploys and evaluates rules first, and flipping the source's gate creates the alert source and attaches the contact point in the same apply (`webhook_configured` keeps `count` plan-time-known; the not-yet-known URL/token are ordinary attribute values) — the full GitOps closure of the alerting last hop. Both modules are cold-start gated on their own provider's readiness.
- **terraform/modules/grafana-cloud-alerting**: Added `webhook_configured` (bool, default `null`) — a plan-time-known override for "a webhook destination exists". Required for composition with resource-sourced webhook credentials (e.g. `incident-io-alert-source`'s `alert_events_url`/`secret_token`): those attributes are unknown until apply, so the previous `webhook_url != null` gate could not drive `count` and plans failed with "Invalid count argument". When null (default) the original null-check applies unchanged — fully backward compatible for callers passing plain variables/literals. The derived gate booleans also strip incidental sensitivity marks (`try(nonsensitive(…), …)`): `coalesce()` unions marks from all arguments, so a sensitive `webhook_url` (a `sensitive = true` variable or another module's sensitive output) would otherwise taint the `enabled`/`notifications_enabled` outputs and fail any root module exporting them with "Output refers to sensitive values".
...
- **terraform/modules/grafana-cloud-alerting**: Generalized the rule engine with two new list inputs, `metric_rules` (Prometheus) and `log_rules` (Loki), so callers can declare arbitrary alert rules — custom PromQL/LogQL `expr`, numeric `threshold`, `reducer`, `for_duration` pending period, `query_from_seconds` lookback, `severity`, `annotations`, and `no_data_state` — appended after the built-in house defaults. The three built-in rules (pod restarts, OOMKilled, error logs) are now expressed in the same object shape as the list inputs and rendered through a single `dynamic "rule"` block per group; two new toggles (`pod_restart_rule_enabled`, `oom_rule_enabled`, both default `true`) allow dropping the metric house defaults to run a purely caller-defined set (the log default already had `error_log_alert_enabled`). The previously fixed "fires when the query reduces to `> 0`" condition is generalized to a per-rule "fires when `reduce(expr) > threshold`", with the reducer configurable per rule; `gt` is intentionally the only comparison operator, keeping the surface minimal. Rule-name uniqueness is enforced within each list (input `validation`) and across the effective built-ins + custom set (a rule-group `lifecycle.precondition`). The metric and log rules remain in separate rule groups so the Loki dependency stays independently disableable, and each group is created only when it holds at least one effective rule. Two new outputs, `metric_rule_names` and `log_rule_names`, expose the rendered rule names. Requires a Terraform-compatible CLI `>= 1.3` for optional object attributes with defaults and the `grafana` provider `>= 2.9.0, < 3.0.0` (provider constraint unchanged).
- **terraform/examples/grafana-cloud-alerting**: Extended the example to exercise the new inputs with documentation-safe placeholders: a `> N` saturation rule, a compound no-progress rule (with `query_from_seconds` widened to cover its range selector), and a count-over-window Loki rule with a non-zero threshold. Added `metric_rule_names` and `log_rule_names` outputs.
- **terraform/modules/aws-eks-grafana-alloy**: Added `annotation_autodiscovery_enabled` (default `false`), mapping 1:1 to the k8s-monitoring chart's top-level `annotationAutodiscovery.enabled`. When enabled, the Alloy metrics collector discovers and scrapes Pods/Services annotated `k8s.grafana.com/scrape: "true"` (honoring the optional `k8s.grafana.com/metrics.*` port/path/scheme/interval/container annotations) and ships those application metrics to the Prometheus destination. Default `false` matches the chart default and keeps the rendered values unchanged until opted into, because enabling autodiscovery widens scrape scope and active-series cost. Advanced sub-configuration — remapping annotations, namespace/label filters, per-port targeting, discovery rules, or metric-processing rules — remains available through the existing `extra_helm_values` passthrough.
- **terraform/examples/aws-eks-grafana-alloy**: Enabled `annotation_autodiscovery_enabled` and documented the consumer-side Pod/Service annotations in the example header.
- **README.md**: Added the observability modules and examples to the root module inventory so the new alerting and Alloy capabilities are discoverable from the project front page.

### Backward Compatibility

All interface additions are additive. The new `grafana-cloud-alerting` list inputs default to `[]` and the two new built-in metric toggles default to `true`, so a caller that sets nothing new renders the same built-in rules; alert rules are nested blocks inside the existing `grafana_rule_group.metrics` / `.logs` resources, so there is no resource-address or `for_each`-key churn. The new rule-group preconditions reject duplicate effective rule names early; existing callers that intentionally set two built-in names to the same value should rename one before upgrading, because duplicate alert names make downstream routing ambiguous. The module now declares a Terraform-compatible CLI constraint of `>= 1.3` because the new typed rule inputs use optional object attributes with defaults; existing state files and supported-CLI callers need no migration. The new `aws-eks-grafana-alloy` toggle defaults to `false`, reproducing the chart default and leaving the rendered values map unchanged until enabled. Existing state files continue to refresh and apply without modification; no migration step is required.

## [1.13.0] — 2026-06-24

### Added

- **terraform/modules/aws-eks-grafana-alloy**: New module that installs the Grafana `k8s-monitoring` (Alloy) Helm chart (v4 `destinations` schema) into an existing Kubernetes cluster, shipping cluster/pod metrics and pod logs to a Prometheus remote-write endpoint and a Loki push endpoint. Vendor-neutral — any Prometheus-remote-write + Loki-push destinations work; the inputs reference Grafana Cloud only as the common case. Two mutually-exclusive credential paths are supported: an inline access token (interpolated into the chart values, so it lands in Terraform state — called out in the variable description) or a reference to a pre-existing Kubernetes Secret (`credentials_secret_name`, used with the chart's `create = false`), which keeps no credential in state and wins when both are set. The release is null-tolerant: it is gated on `enabled` AND both push endpoints AND a credential source being present, so it stays inert (`count = 0`) on cold-start / pre-credential plans instead of erroring. Feature toggles (`clusterMetrics`, `clusterEvents`, `podLogsViaLoki`), a pinned `chart_version`, a `helm_timeout`, and an `extra_helm_values` passthrough (appended after module-managed values; last value wins) are exposed as inputs. Requires only the `helm` provider.
- **terraform/examples/aws-eks-grafana-alloy**: New example demonstrating the inline-token credential path against a Grafana Cloud backend with documentation-safe placeholders (placeholder push URLs, instance IDs, and token); a header note points at the state-safe pre-existing-Secret alternative.
- **terraform/modules/grafana-cloud-alerting**: New module that provisions Grafana-managed workload-health alert rules routed to a generic webhook contact point (incident.io / PagerDuty / Opsgenie / Slack-webhook / …). It ships house-default rules for three signals — pod restarts and OOMKilled (both Prometheus / kube-state-metrics) and an optional error-log rule (Loki). The metric rules and the log rule live in separate rule groups so the log rule and its Loki dependency can be disabled (`error_log_alert_enabled = false`) for workloads that do not ship logs to Loki. Per-rule simplified `notification_settings` route to the contact point without taking over the org-wide root notification policy (requires Grafana 10.4+ and the `grafana` provider `>= 2.9.0, < 3.0.0`). Every workload-specific value (namespace, labels, severities, the log-match regex, datasource names, annotations, pending periods) is an input; datasource names fall back to the Grafana Cloud auto-provisioned names derived from a stack slug. Null-tolerant: gated on `enabled` AND `webhook_url`, so it stays inert (`count = 0`) until a webhook is supplied. Because the datasource lookups run at plan time, the `enabled` input is documented as the place for callers to fold in provider-readiness.
- **terraform/examples/grafana-cloud-alerting**: New example wiring the alert rules to a generic webhook contact point with documentation-safe placeholders (`example-workload` namespace, placeholder webhook URL/token, and a `grafana_stack_slug` for datasource-name derivation), including the plan-time provider-readiness caveat.
- **toolbox/operator-tools/check-env-contract-drift.sh**, **hydrate-stack-secrets.sh**, and **lib/contract.sh**: New env-file *contract* convention, the two scripts that implement it, and the sourceable grammar they share. A contract is a committed template (`<secure-dir>/contracts/<stack>.env.example`) that enumerates the variable *names* a real `<env>-<region>-<stack>.env` may define — never values. `lib/contract.sh` parses the two annotations (an uncommented `KEY=` line is REQUIRED; a commented `# KEY=` line is OPTIONAL) and is sourced by both scripts so the grammar is defined once. `check-env-contract-drift.sh` (read side) validates shape: for each real `.env` it locates the contract and enforces REQUIRED ⊆ real and real ⊆ (REQUIRED ∪ OPTIONAL), and additionally fails when a contract that declares REQUIRED keys has no matching real file — catching the silently-missing-`.env` class that otherwise surfaces as an opaque provider-auth error deep inside a plan. Exit 0 = clean, 1 = drift, 2 = usage error, so it suits a CI pre-flight. `hydrate-stack-secrets.sh` (write side) is the inverse: driven by the same contract, it upserts operator-supplied secret VALUES (read from the process environment by contract key name) into the real `.env` — chmod 600, symlink-safe, position-preserving, and never printing values. Because it only ever writes contract-declared keys it cannot introduce drift. Repo-agnostic: the secure-files directory and the key-to-file mapping live in the contract, so any caller (CI, an operator shell, a rotation tool) drives both scripts identically.

### Changed

- **toolbox/operator-tools/README.md**: New "Env-file contracts" section documenting the contract convention and both scripts, and the Layout block now lists `check-env-contract-drift.sh`, `hydrate-stack-secrets.sh`, and `lib/contract.sh`.
...
- **toolbox/operator-tools/compare-dns-snapshots.sh**: New operator tool that diffs two snapshots produced by `capture-dns-snapshot.sh`. Strips header comment lines (which carry capture timestamps and would always differ) before comparison. Exit 0 = snapshots are 1:1 identical, exit 1 = drift detected, exit 2 = invocation error. Intended use: gate apply on the pre-apply snapshot equalling the pre-plan snapshot (proving nobody mutated DNS during the planning window) and gate cutover-success on the post-apply snapshot equalling the pre-plan snapshot (proving the apply was a true no-op against live DNS). Drift output is the standard `diff(1)` line-marker format so the failing record names and values are visible without further tooling.
- **toolbox/operator-tools/verify-dns-1to1.sh**: New operator tool that performs semantic 1:1 verification of authoritative DNS state against the BIND-format zone exports. The contract is per `(fqdn, type)` RRset, not per BIND line: for every RRset described in the exports, the live RRset (the full set returned by `dig +short`, normalized per type) must EQUAL the BIND-declared RRset under set-equality. Both *missing* values (in BIND, absent from live) and *extra* values (in live, absent from BIND) count as drift and are individually reported via `comm -23` / `comm -13`. The set-equality semantic (rather than per-BIND-line equality) is critical for multi-value RRsets — multi-MX (Cloudflare Email Routing or Google Workspace's three-record MX), multi-A round-robin, multi-NS at apex, and split-SPF TXT all have N>1 values per RRset, and a per-line comparison falsely flags every such RRset as drift even when DNS is configured exactly as declared. The extra-value detection covers the most operationally dangerous failure mode of a DNS-import migration: a record that exists live (often a vestigial entry from a previous DNS provider, a hand-edit somebody forgot to delete, or an out-of-band addition during the migration window) but was not in the exported zone file, which a one-way "every BIND record is reachable" check would silently miss. Proxied records (records carrying a `; cf-proxied:true` comment in the export) take a separate path: the live answer must be non-empty, but byte-equality is not enforced because the live answer is a Cloudflare anycast address that the export cannot pre-compute; the alive-only check is the strongest invariant the tool can express for proxied RRsets without speaking the Cloudflare API. SOA and apex NS are skipped — both are managed at the registrar/parent-zone level and BIND export shape varies across providers; `capture-dns-snapshot.sh` captures them for the snapshot pair so drift on those still surfaces in the byte-stable diff path. Bound: the tool catches every divergence within RRsets that the BIND export describes, but does not enumerate live records whose `(fqdn, type)` is absent from the export entirely (public DNS does not support enumerate-all-records over `dig` — only AXFR or a vendor API does, and both are deliberately out of scope for this `dig` + `awk` + `comm` toolchain). Therefore the export must be a complete dump of the zone; for Cloudflare, the dashboard's "Export DNS records" button produces that. Exit 0 = every RRset 1:1 + every proxied RRset alive, exit 1 = any drift or any proxied RRset not resolving. Nameserver configurable via `NAMESERVER`.

### Changed

- **terraform/modules/cloudflare-domain-baseline**: The `dns_records` input now accepts an optional `comment` per record. The value is passed through verbatim to the Cloudflare record metadata, allowing operators to declare the human-readable per-record labels that Cloudflare stores alongside each record (visible in the dashboard and on the API). Without this passthrough, importing existing records into Terraform-managed state surfaces every populated comment as a `comment = "…" -> null` planned-deletion on first plan, and apply would wipe operator-authored labels. The new field accepts any free-form string. The three `cloudflare_dns_record` resources (`standard`, `txt`, `caa`) wire it to the provider's `comment` argument; absent input behaves as before (no comment set).
- **terraform/modules/cloudflare-mail-foundation**: The `mx_records`, `dkim_records`, and `verification_records` list inputs each gained an optional `comment` field per item. Two new top-level scalar inputs `spf_comment` and `dmarc_comment` cover the singleton apex SPF and `_dmarc` records. All five `cloudflare_dns_record` resources (`mx`, `spf`, `dkim`, `dmarc`, `verification`) wire the comment through to the provider for the same reason as `cloudflare-domain-baseline`. Additionally, the `spf_value` and `dmarc_value` regex validations were relaxed to accept an optional matched pair of literal surrounding double-quotes (`"v=spf1 …"`, `"v=DMARC1; …"`). Cloudflare's API stores TXT content byte-for-byte and does not strip caller-supplied surrounding quotes — records originally created through the dashboard or through third-party tools that wrap TXT values in literal quotes can now be imported into module-managed state without triggering a planned content rewrite (which would change the stored byte-string even though dig wire-output is unchanged). A second symmetric validation rejects mismatched leading/trailing quote pairs (`"v=spf1 …` or `v=spf1 …"`) since either form would be a typo, not a legitimate stored value. The module still applies `trimspace()` to the input (whitespace stripping, not quote stripping); quotes pass through unchanged. Consumers that pass unquoted values see no behavior change.
- **terraform/modules/cloudflare-redirect-domain**: No interface change. (Earlier `[Unreleased]` drafts of this entry described a `lifecycle { ignore_changes = [modified_on, comment_modified_on] }` addition to the `apex_placeholder` and `www_placeholder` resources; that change was reverted before release because OpenTofu correctly rejects `ignore_changes` on provider-computed-only attributes — the rule has no effect and emits a `Redundant ignore_changes element` warning on every plan. The original concern that motivated the rule — perpetual `modified_on` plan churn — was misdiagnosed: Terraform does not surface diffs on pure computed attributes; the `~ modified_on -> (known after apply)` line in plan output is a consequential side-effect of in-place updates driven by other attribute changes, not a causal driver. There is no churn to suppress.)
- **terraform/modules/cloudflare-preview-website**: No interface change, same reason as `cloudflare-redirect-domain` above.
- **toolbox/operator-tools/README.md**: Layout block and section listings now include the three DNS migration tools (`capture-dns-snapshot.sh`, `compare-dns-snapshots.sh`, `verify-dns-1to1.sh`) under a new "DNS migration" subsection, matching the existing convention for the other tool families in the file.

### Rationale

DNS migrations into Terraform are uniquely high-stakes because the apply step touches an external authoritative system that downstream services depend on for liveness. The standard "import then plan-zero-diff" pattern proves the imported state matches the declared configuration but does not by itself prove that *between import and apply* nobody else mutated live DNS. The three new operator tools close that gap with a deterministic capture-compare loop independent of the Terraform state file: the rollback baseline is a text file produced by `dig`, not by `terraform show`, so it is unaffected by state-file corruption, provider bugs, or partial applies. The toolchain is intentionally simple — `dig` plus `awk` plus `diff` — so it runs identically in CI, on operator laptops, and in incident-response shells. The companion module change (passthrough of `comment` on every `cloudflare_dns_record` resource in the four affected modules) closes the only real source of import-time plan diff that was attributable to the modules themselves: existing records carry operator-authored labels, and without a passthrough, importing them into the modules' state surfaced every label as a planned deletion.

### Fixed

- **toolbox/operator-tools/verify-dns-1to1.sh** (Cloudflare BIND-export inline-comment format): The pre-release implementation expected the proxied-state marker to appear as a bare ` ; cf-proxied:<bool>` suffix at end of line — Cloudflare's older export format. Current Cloudflare BIND exports emit the marker as `cf_tags=cf-proxied:<bool>` and may include an arbitrary operator-authored comment before it (`; operator-authored-label cf_tags=cf-proxied:false`). The old regex matched neither modern form, so proxied state was always parsed as `none`, the inline-comment trail was never stripped from the value, and every CNAME/MX/SRV record with a populated comment surfaced as a false-positive DRIFT against the un-commented dig response. The fix detects proxied state by matching `cf_tags=cf-proxied:<bool>` anywhere in the raw line, then strips the comment via a quote-aware character walk (see below).
- **toolbox/operator-tools/verify-dns-1to1.sh** (BIND comment stripper for TXT records containing `;`): The naive `sed 's/[[:space:]]*;.*$//'` greedy-stripped from the first `;` onwards — but TXT record values legitimately contain `;` characters (DMARC policies use them as tag separators: `v=DMARC1; p=quarantine; rua=…`; SPF and verification TXTs occasionally do too). Every such TXT was silently truncated at the first internal semicolon (e.g. `_dmarc` was truncated to `"v=DMARC1`) and surfaced as DRIFT against the full, un-truncated live answer returned by `dig +short TXT`. The fix replaces the sed with an awk character-walker that tracks double-quote state and cuts at the first `;` outside quotes — BIND-spec-correct behavior.
- **toolbox/operator-tools/verify-dns-1to1.sh** (proxied-CNAME liveness query type): The pre-release implementation queried `dig +short @<ns> <fqdn> <bind-declared-type>` for the liveness check on proxied RRsets, which returned an empty answer for every proxied CNAME because Cloudflare's edge proxy flattens proxied CNAMEs (CNAME flattening / orange-cloud behavior) and the wire response carries an A (and optional AAAA) RR — never a CNAME RR — so `dig +short CNAME` filters out the only useful answer and returns nothing. Every proxied CNAME was therefore reported as `NOT RESOLVING` even though the FQDN was reachable end-to-end via the browser. The fix queries A first and falls back to AAAA, asserting "Cloudflare's edge has at least one address-type record live for this hostname" — the strongest liveness invariant expressible without speaking the Cloudflare API.

### Backward Compatibility

Toolbox additions are purely additive; no existing tool is modified. The module input additions are all optional with `null`/absent defaults — existing consumers that do not set `comment` on records, `spf_comment`, or `dmarc_comment` see no schema or behavior change. Outputs, resource addresses, and `for_each` keys are unchanged across all four affected modules. Existing state files continue to refresh and apply without modification. The three `verify-dns-1to1.sh` fixes are bugfixes against the pre-release `[Unreleased]` entry — no previous release shipped the broken behavior, so no consumer downgrade or migration step is required.
...
- **terraform/modules/keycloak-bootstrap-admin**: Bootstrap service-account client role grant expanded from `["manage-users"]` to `["manage-users", "manage-realm"]` in both `realm_role_mapping_json` and `realm_service_account_user_json` outputs. Both are granular roles inside the `realm-management` client; this is a TWO-role grant, NOT a switch to a composite. (Earlier draft of this entry incorrectly described `manage-realm` as a composite that includes `manage-users` — that is wrong. `manage-realm` and `manage-users` are sibling granular roles inside the `realm-management` client; neither implicitly grants the other. The `realm-admin` role is the actual composite that contains both, plus impersonation/manage-events/manage-clients/manage-identity-providers/manage-authorization.) Motivation: enables consumers to call `POST /admin/realms/{realm}/partialImport` from their bootstrap mechanism, which is the only Admin REST endpoint that preserves client-supplied user `id`s on existing realms — `POST /users` silently auto-generates ids and breaks the deterministic-`sub` contract this module's `admin_user_id` output establishes. The day-zero `POST /admin/realms` (bulk realm create) path also preserves ids but is only available on the first apply; once the realm exists, partialImport is the only path. Upstream KC issue: [keycloak/keycloak#12454](https://github.com/keycloak/keycloak/issues/12454). Both granular roles are necessary because: (a) `manage-users` alone authorises POST /users + role-mappings + attribute-writes but returns 403 on partialImport; (b) `manage-realm` alone authorises partialImport but returns 403 on user CRUD / role-mapping / attribute writes (which the bootstrap script also performs for the SKIPPED idempotent-re-run path). HIPAA / SOC2 / FedRAMP rationale for NOT switching to the `realm-admin` composite: realm-admin includes impersonation (the bootstrap service account would be able to act as any user), manage-events (audit log mutation), manage-clients, manage-identity-providers, manage-authorization — over-scoped for the documented bootstrap operations and inappropriate for an automated service account in restricted-compliance deployments. The two-granular-role grant is the minimum-necessary set: every Admin REST endpoint the bootstrap script touches is authorised, nothing else is. Privilege boundary unchanged at the realm level: both roles are scoped to one realm; no cross-realm or master-realm access. Backward compatibility: `manage-users` was already in the prior grant and remains; existing consumer scripts using only POST /users continue to work unchanged. The added `manage-realm` only enables the additional partialImport capability. Module API surface unchanged: same outputs, same inputs, same JSON splice shapes (only the role array inside the JSON changed). README's `Pattern A — Kubernetes Job` section now recommends `partialImport` as the canonical bootstrap call with `ifResourceExists: "SKIP"` for idempotent re-runs; consumers using the prior `POST /users` pattern remain functional but will hit the day-zero-vs-existing-realm fragility documented in KC#12454.
...
- **toolbox/operator-tools/render-streaming-bundle.sh** (`flink_current_catalog` / `flink_current_database` output reads): Optionally read two new scalar outputs holding the Flink SQL catalog and database display names. When both are present, the renderer additionally emits `FLINK_CATALOG_NAME=<env-display-name>` and `FLINK_DATABASE_NAME=<cluster-display-name>` into the Flink block. Consumers pass these into `spec.properties.sql.current-catalog` / `sql.current-database` on `POST /sql/v1/.../statements` so Flink SQL can resolve unqualified table references (topic names) — without them a statement reaches `phase=FAILED` at plan time with `Table '...' does not exist, may be on a private cluster, or you do not have permission to access it. Using current catalog '' and current database ''.` even though the topic exists in Schema Registry and the deploy SA has both `Assigner` on the runner SA and `FlinkDeveloper` on the environment. The vendor `confluent flink statement create` CLI derived these from `~/.confluent/config.json` current-context; the REST API has no equivalent implicit context, so the curl + Basic-auth consumer pattern this renderer enables must carry the values explicitly. Gated independently of the `flink_organization_id` block: stacks that expose path-pre-rendering outputs but not catalog/database (or vice versa) still get usable Flink blocks. Both values are required together to emit either — a catalog without a database would resolve to "table not found in catalog X" rather than the clearer empty-context error. Defensive `null`-coercion mirrors the existing pattern for the other six `flink_*` scalar reads.
...
Fully additive on the rendered `.env` file shape — pre-v1.9.1 consumer scripts that only read `FLINK_REST_ENDPOINT` / `FLINK_API_KEY` / `FLINK_API_SECRET` ignore the five new lines (`FLINK_PLATFORM`, `FLINK_ORGANIZATION_ID`, `FLINK_STATEMENTS_PATH`, `FLINK_CATALOG_NAME`, `FLINK_DATABASE_NAME`) and continue to work unchanged. Stacks without any of the three optional outputs (`flink_organization_id`, `flink_current_catalog`, `flink_current_database`) produce a Flink block identical to v1.8.6's shape plus only the `FLINK_PLATFORM=confluent_cloud` line; each output pair is gated independently, so a stack can opt into statement-path pre-rendering without catalog/database emission, or vice versa. Consumers that need either pair must expose the matching outputs in their streaming module to opt in.
...
- **terraform/modules/aws-eks-aurora-cluster/resources.tf** (`aws_rds_cluster.this`, `aws_rds_cluster_instance.writer`, `aws_rds_cluster_instance.reader`): Add `engine_version` to `lifecycle.ignore_changes` on the cluster and both instance resources. AWS Aurora defaults `auto_minor_version_upgrade = true` at the instance level, so AWS-driven minor bumps (e.g. `16.4 → 16.11`) land out-of-band; the next `terraform apply` would otherwise see drift and attempt to `ModifyDBCluster`/`ModifyDBInstance` back to the var-pinned minor, which AWS rejects with `InvalidParameterCombination: Cannot upgrade aurora-postgresql from <live> to <stale>` (minor downgrades are not permitted), breaking every subsequent apply on the consuming stack until an operator manually chases live by editing `var.engine_version`. The instance ignore_changes blocks are required because both instances wire `engine_version = aws_rds_cluster.this.engine_version`, which carries the (now-ignored, possibly stale) TF-state value rather than live AWS state — a cluster-only ignore would still let the provider attempt the same downgrade against each instance. Major-version bumps remain operator-driven and non-destructive in-place via `ModifyDBCluster` + `ModifyDBInstance`; the inline comment on the cluster resource documents the four-step lift-and-restore procedure (lift on cluster + writer + reader simultaneously, add `allow_major_version_upgrade = true`, bump `var.engine_version`, apply, restore the three blocks). The procedure is explicitly contrasted with `-replace=aws_rds_cluster.this`, which is destructive and drops every tenant database — analogous to the existing `master_password` Scenario B vs Scenario C distinction in the secret-version comment block.

### Added

- **operator-tools/render-streaming-bundle.sh**: Render `FLINK_*` block when `workload_flink_api_*` outputs are present. Workload iteration now unions the Kafka and Flink key-id map outputs (`workload_kafka_api_key_ids` + `workload_flink_api_key_ids`) so a Flink-only submit service account renders `<workload>.env` containing only `FLINK_REST_ENDPOINT`, `FLINK_COMPUTE_POOL_ID`, `FLINK_RUNNER_SERVICE_ACCOUNT_ID`, `FLINK_ENVIRONMENT_ID`, `FLINK_API_KEY`, `FLINK_API_SECRET` — no empty `KAFKA_*`/`SCHEMA_REGISTRY_*` lines. When a workload appears in multiple maps, blocks render in Kafka → SR → Flink order separated by blank lines. Pre-v1.8.6 stacks (no Flink outputs present) render their existing Kafka (+ optional SR) bundle unchanged — `tofu output -json workload_flink_api_key_ids` falls back to `{}` when the output does not exist, so the union degenerates to the original Kafka-only key set. The new Flink-side output names mirror the existing `workload_<tech>_api_key_ids` convention already used for Kafka and Schema Registry rather than introducing a use-case-specific prefix.
  - Renderer restructured to append each block (Kafka / SR / Flink) independently and only when the workload has a non-empty credential for that block. Replaces the earlier write-then-truncate approach where a Flink-only workload's `: > env_file` reset would also erase an already-appended SR block if the workload happened to hold an SR key without a Kafka key — a hidden side effect between blocks. Workloads that resolve to empty credentials in all three maps (possible when a map literally contains `"foo": null`) are now skipped with a log line and counted separately in the summary output, rather than producing a zero-length 0600 file.
  - Defensive coercion of JSON/raw `null` from `tofu output` for the new Flink fetches: `tofu output -json` on an output that evaluates to null (natural shape for `value = try(resource.flink[0].id, null)` wrapping an optional resource) prints the JSON literal `null` with exit 0, which would otherwise make `jq --argjson f "null" '($k + $f) | keys[]'` fail and short-circuit the whole renderer with a misleading "No workload credentials found" error. Coercion applies to all six new Flink fetches (`workload_flink_api_key_ids`, `workload_flink_api_secrets`, and the four `flink_*` scalar outputs) and mirrors the "null" → empty normalization that `lib/render-helpers.sh::read_tf_output` has applied to the `make output` table-parser path since v1.8.3.
...
- **toolbox/operator-tools/README.md** (`render-streaming-bundle.sh`): Rewrite the entry to document the three-block composition (Kafka / SR / Flink), the union-of-maps workload discovery against `workload_kafka_api_key_ids` + `workload_flink_api_key_ids`, and the skip-on-empty-credentials behavior. Previous text described only the Kafka + SR shape and did not mention the Flink block added in this release.
- **toolbox/operator-tools/render-streaming-bundle.sh** (header docstring): Expand the `Output:` section to document all three possible blocks, their per-workload gating conditions, and the Flink-only / no-credentials cases; the previous header still described only the Kafka + SR shape.
