...
# tls, streaming) and writes correctly-formatted .env files directly into a
# target service project's .secure_files/ directory.
#
# This replaces the manual workflow of running individual render scripts,
# copying outputs, and renaming files.
#
# Usage:
#   ./render-service-bundle.sh --service <ingress|structuring> \
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *)
      ...
      echo "Usage: $0 --service <ingress|structuring> --env <env> --region <region> --infra-dir <path> --target-dir <path>" >&2
      exit 1
      ;;
  esac
done
...
for dir in "$CLOUD_DIR" "$DATA_DIR" "$STREAMING_DIR"; do
  if [[ ! -d "$dir" ]]; then
