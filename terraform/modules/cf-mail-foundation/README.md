...
When composing this module with `cf-domain-baseline`, keep all mail-related DNS out of `cf-domain-baseline.dns_records`. Duplicating SPF, DKIM, DMARC, MX, or verification TXT records across both modules can create invalid or misleading DNS state.
...
```hcl
module "cf_mail_foundation" {
  source = "../../modules/cf-mail-foundation"
