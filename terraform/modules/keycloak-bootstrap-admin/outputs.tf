...
output "realm_role_mapping_json" {
  description = "JSON-encoded clientScopeMappings entry ready to merge into a Keycloak realm JSON. Declares that JWTs issued for the bootstrap client may carry two granular roles from the realm-management client: 'manage-users' (required for user CRUD, role-mappings, attribute writes) and 'manage-realm' (required for partialImport — the only Admin REST endpoint that preserves client-supplied user `id`s on existing realms; keycloak/keycloak#12454). This is a scope setting, not a role assignment. Load-bearing because the bootstrap client is configured with fullScopeAllowed=false — without this entry the JWT would carry no roles even after realm_service_account_user_json grants them to the user. manage-users and manage-realm are SIBLING granular roles inside the realm-management client (NOT a parent-child composite); the realm-admin composite role would grant both AND impersonation/manage-events/manage-clients/manage-identity-providers/manage-authorization, which is over-scoped for the documented bootstrap operations and inappropriate for HIPAA/SOC2/FedRAMP-aligned deployments. Splice via jsondecode() + merge() into 'clientScopeMappings' in your realm-import templating."
  value       = jsonencode(local.realm_role_mapping)
}

output "realm_service_account_user_json" {
  description = "JSON-encoded user entry ready to splice into the 'users' array of a Keycloak realm JSON. THIS is the load-bearing role grant for the bootstrap mechanism: it assigns two granular realm-management roles ('manage-users' + 'manage-realm') to the auto-created service-account user 'service-account-$${bootstrap_client_id}'. Without this entry the JWT obtained via client_credentials grant carries no roles and Admin API calls return 403. manage-users is required for user CRUD and role-mapping/attribute writes; manage-realm is required for partialImport (the only Admin REST endpoint that preserves client-supplied user `id`s on existing realms; keycloak/keycloak#12454). Privilege boundary: both roles are scoped to a single realm. The bootstrap client cannot impersonate users, manage other clients, manage events, manage identity providers, or manage authorization — those are separate granular roles inside the realm-management client and are deliberately not granted. The realm-admin composite role would grant impersonation; granting it would be inappropriate for an automated service account in HIPAA/SOC2/FedRAMP-aligned deployments. Splice via jsondecode() in your realm-import templating."
