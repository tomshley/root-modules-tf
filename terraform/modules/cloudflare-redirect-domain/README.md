...
- `redirect_ruleset_id`
  - Rulesets API ruleset ID that implements redirect behavior for the source zone.
...
- This module intentionally does not manage mail behavior. If the legacy domain must keep receiving mail, compose `cloudflare-mail-foundation` using the `zone_id` output.
