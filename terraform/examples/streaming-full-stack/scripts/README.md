...
Real consumer repos (e.g. `ami-infrastructure`) typically add a thin wrapper script like `scripts/operator-session.sh` that chains all session tools for a given stack prefix:
